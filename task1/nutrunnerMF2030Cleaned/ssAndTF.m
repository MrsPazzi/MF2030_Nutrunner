%% Task1 - Symbolical representation State-space
clc, clear, close all;
syms R L K_E K_M J b_m
...
%% Task1 - Symbolical representation Transfer function
clc, clear, close all;
syms R L K_E K_M J b_m s
...
%% Task2 - Numeric representation (State-space and Transfer function) + step response + tau
...
[y,t] = step(stateSpace,opt);

G = tf(stateSpace)         % speed/voltage transfer function (uses ss object)
dcg = dcgain(G);           % steady-state speed per volt (rad/s per V)

no_load_speed = dcg * U_N * (60 / (2 * pi)) % [rpm]
...
stateSpace = ss(A,B,C,D);  % M, U --> [omega, i]
G = tf(stateSpace)         % [omega, i]
dcg = dcgain(G);   
...
%% Task 2 - Numeric representation State-space + step response (current)
...
stateSpace = ss(A,B,C,D); % Creates a state-space object
...
%% Task5 - Calculated gear ratio

clc
run 'motor_specs.m' 

totalEfficiency = 1;               % [-] (eta_g)

torqueSection1 = calculated_stall; % [Nm]
torqueSection2 = maximumTorque;    % [Nm]

totalGearRatio = torqueSection2 / (torqueSection1 * totalEfficiency) % [rpm] (n)

gearRatioSection1 = sqrt(totalGearRatio) % [rpm] (n_g1)
...
volumeHorisontalAngle = [(24.5 * (((10/2)^2)*pi)), ...
                         (19.6 * (((11/2)^2)*pi)), ...
                         (13.9 * (((14/2)^2)*pi)), ...
                         (13.1 * (((19.6/2)^2)*pi))] .* 1E-9; % [m^3]
volumeVerticalAngle = [(7.042 * (((27.7/2)^2)*pi)), ...
                       ((9.55^2) + (9.55^2))] .* 1E-9;        % [m^3]
...
radiusSquaredMiddleAxis = [(5.9/2)^2 , ...
                           ((9/2)^2 + (11/2)^2)] .* 1E-6;              % [m^2]
volumeMiddleAxis = [((5.9/2)^2 * (40.3 - 16.2)), ...
                    (((11/2)^2 * 16.2) - ((9/2)^2 * 14))] .* 1E-9;     % [m^3]
m_middleAxis = densitySteel .* volumeMiddleAxis;                       % [kg]
J_middleAxis = sum(((1/2) * m_middleAxis) .* radiusSquaredMiddleAxis); % [kgm^2]
...
A6 = [ 0, 0, 1, 0;
      0, 0, 0, 1;
     -k_t/(J_gm*n^2), k_t/(J_gm*n), (((-K_E*K_M)/R) - b_m - (d_t/(n^2)))/J_gm, d_t/(J_gm*n);
...
k_j = (A_M8*E_steel) / L_free;     % [Nm/rad]

k_theta = k_j * (L_p / (2 * pi))^2 % [Nm/rad]
...
stateSpaceFouthOrder = ss(A8,B6,C6,D6)